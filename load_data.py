...
def load_text_dataset(data_path, seed=123):
#   file = '_SELECT_top_10_da_intDocumentoAnexoId_id_da_dataCadastro_dataCri_201905191409_20190517.csv'
    #file = '_SELECT_top_10_da_intDocumentoAnexoId_id_da_dataCadastro_dataCri_201905191426_20190429-0506.csv'
    file = 'docs.csv'
    columns = ['classe','texto'] # 0 - label, 1 - title, 2 - body.
    data = _load_and_shuffle_data(
            data_path, file, columns, seed, header=0)

    y=data['classe']
    x=data['texto']
    #print(y)
    y, uniques = pd.factorize(y)
    pickle.dump(uniques, open("labels.pickle", "wb"))
    #print(y)
    #print(uniques)
    #print((x.values[0]))
    
    x = pd.Series(data=map(lt.cleanhtml,x.values))
    print('<----------------------->')
    #print(x[0])
    print('<----------------------->')
    train_texts, test_texts, train_labels, test_labels = train_test_split(x, y, test_size=0.33, random_state=42)

    return ((train_texts, train_labels), (test_texts, test_labels)),uniques
...
def _load_and_shuffle_data(data_path,
    ...
    data = pd.read_csv(data_path, usecols=cols, sep=separator, header=header, encoding='latin-1',skip_blank_lines=True)
